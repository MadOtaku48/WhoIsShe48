#!/usr/bin/env python3
"""
Runtime hook for InsightFace to properly set model paths in PyInstaller bundle (Windows version)
"""
...
if hasattr(sys, '_MEIPASS'):
    ...
    # On Windows, set both HOME and USERPROFILE
    os.environ['HOME'] = bundle_dir
    os.environ['USERPROFILE'] = bundle_dir
    ...
    print(f"[Runtime Hook] USERPROFILE set to: {os.environ['USERPROFILE']}")
    ...
    if os.path.exists(buffalo_l_path):
        ...
        try:
            ...
            print(f"[Runtime Hook] Error listing model files: {e}")
    else:
    ...
    if os.path.exists(insightface_dir):
        ...
        try:
            ...
            print(f"[Runtime Hook] Error walking directory: {e}")
    
    # Windows-specific: Set ONNX Runtime providers
